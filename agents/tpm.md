...
### Wave 너비 시뮬레이션 (강제 — 직선형이면 재분해)
DAG를 위상정렬해 wave별 ready 셋 크기를 산출한다.
...
평균 너비: {값} (목표: 팀 동시 인원의 70% 이상). **모든 wave 너비 ≤ 2면 직선형 = 분해 실패** → 재분해.
...
### 분해 재검토 트리거 (강제 — 충족 못하면 산출물 인계 금지)

다음 중 하나라도 해당하면 **재분해 전 산출물을 인계하지 않는다**:
- **모든 wave 너비 ≤ 2 (직선형 DAG)** — 병렬 불가. 독립 병목을 쪼개거나 도메인 경계를 다시 긋는다.
- **후행 카운트 ≥ 3 티켓** — **독립적인** 하위 단위로 쪼개 fan-out을 넓힌다.
- **연관(결합)된 병목이 여러 티켓으로 흩어짐** — `rules/ticket-guide.md` "연관 병목은 하나로 묶는다"에 따라 하나의 선행 티켓으로 합친다 (직렬 사슬·같은 파일 충돌 방지).

자세한 기준은 `rules/ticket-guide.md` "Fan-out 너비 목표" 참조.
