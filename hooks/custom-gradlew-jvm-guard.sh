#!/usr/bin/env bash
# PreToolUse hook (Bash matcher) — ./gradlew 실행 직전 모듈 CLAUDE.md 의 JVM 핀과 $JAVA_HOME 비교.
...
# fallback 은 스크립트 위치(.claude/hooks/) 기준으로 두 단계 위를 역산해 워크스페이스 루트를 잡는다.
...
# 루트에서 직접 빌드 차단
if [[ "$EFFECTIVE_CWD" == "$ROOT" ]]; then
  echo "🛑 워크스페이스 루트에서는 ./gradlew를 실행할 수 없습니다. 통합 wrapper 없음." >&2
  echo "→ 모듈 디렉토리로 cd 후 실행하세요. 어느 모듈인지 모르면 /custom-module-router 사용." >&2
  exit 2
fi
...
relpath="${EFFECTIVE_CWD#"$ROOT"/}"
module="${relpath%%/*}"
claudemd="$ROOT/$module/CLAUDE.md"

if [[ ! -f "$claudemd" ]]; then
  # 알려지지 않은 디렉토리 — 차단하지 않고 통과 (사용자 PoC 등)
  exit 0
fi
...
if [[ "$current" != "$required" ]]; then
  cat >&2 <<EOF
🛑 JDK 미스매치: 모듈 '$module' 요구 = JVM $required, 현재 = JVM $current
