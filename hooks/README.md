...
| 파일 | 시점 | matcher | 동작 | 차단? |
|---|---|---|---|---|
| `custom-session-start.sh` | SessionStart | `""` | cwd 기반 모듈/JDK/시크릿 점검 1회 안내 | — |
| `custom-gradlew-jvm-guard.sh` | PreToolUse | `Bash` | `./gradlew` 직전 모듈 CLAUDE.md JVM 핀 vs `$JAVA_HOME` 비교 | ✅ 미스매치 시 |
| `custom-check-commit-prefix.sh` | PreToolUse | `Bash` | `git commit -m` 메시지에 `[GRT-####]`/`[BE-####]`/`[LIVEISSUE-####]` 강제 | ✅ |
| `custom-block-git-push.sh` | PreToolUse | `Bash` | `git push` 직전 사용자 승인 프롬프트(ask). --force / --force-with-lease 사용 시 reason 에 경고 노출 | 🔔 (ask) |
| `custom-block-mainline-merge.sh` | PreToolUse | `Bash` | `gh pr merge` / `git merge\|rebase (main\|dev\|stage-*)` 명시 확인 | ✅ |
| `custom-block-infra-write.sh` | PreToolUse | `Bash` | `kubectl`/`aws`/`terraform` write 동사 차단 | ✅ |
...
| `custom-confirm-mcp-write.sh` | PreToolUse | `mcp__claude_ai_(Slack\|Atlassian\|Gmail\|Google_Calendar\|Google_Drive\|Figma)__... \| mcp__datadog-mcp__... \| mcp__grafana-dev__...` | Slack 발송 / Jira·Confluence·Gmail·Calendar·Drive 쓰기 + Figma 디자인 파일 변경 + Datadog 대시보드·노트북·모니터 / Grafana 대시보드·인시던트·알림 변경 직전 사용자 승인 프롬프트(ask). Approve → 호출 진행, Deny → 호출 취소(자동 재시도도 다시 프롬프트) | 🔔 (ask) |
| `custom-avro-change-reminder.sh` | PostToolUse | `Edit\|Write\|MultiEdit` | `.avsc`·`.avdl` 변경 후 호환성/배포 순서 명시 리마인더 (JSON `additionalContext` 로 Claude 컨텍스트 주입) | — |
| `custom-detekt-touch-reminder.sh` | PostToolUse | `Edit\|Write\|MultiEdit` | `.kt`/`.kts` 변경 후 해당 모듈 `./gradlew detekt` 실행 안내 (JSON `additionalContext` 로 Claude 컨텍스트 주입) | — |
...
5. 검증: `for f in .claude/hooks/*.sh; do bash -n "$f"; done`

## 차단 vs 승인 요청 vs 안내 정책

- **차단(exit 2)** — 운영 사고로 직결되는 위반(인프라 write, prefix 누락, JDK 미스매치, 비밀 파일 편집). stderr 메시지가 Claude 컨텍스트로 전달돼 자기 수정 유도.
- **승인 요청(exit 0 + `permissionDecision: "ask"` JSON)** — PreToolUse 전용. Claude Code 가 사용자에게 호출 승인 프롬프트를 띄운다. Approve → Claude 가 직접 호출 진행, Deny → 호출 취소(자동 재시도도 동일 프롬프트 통과 필요). `custom-confirm-mcp-write.sh` / `custom-block-git-push.sh` 가 사용:
    ```json
...
- **안내(exit 0)** — 이벤트별로 컨텍스트 도달 방식이 다르다 (공식 문서 기준):
  ...
  - **PreToolUse / PostToolUse**: plain stdout 은 debug log 에만 기록되고 Claude 가 못 본다. Claude 에게 안내가 도달해야 하면 JSON 형식으로 출력해야 한다:
    ...
    ```
    `custom-avro-change-reminder.sh` / `custom-detekt-touch-reminder.sh` 가 이 형식을 사용한다.
